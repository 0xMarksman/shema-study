...
A 365-day whole-Bible reading plan and community app built from a **Messianic Jewish perspective** using the Tree of Life Version (TLV). Installable as a PWA on iPhone, Android, and desktop.

## Features

- **Four parallel reading tracks** — Tanakh, Psalms, Proverbs, and B'rit Chadashah
- **Daily study questions** — observation, word study, Messianic connection, application, discussion
- **Rich-text answers** — B/I/U formatting with tappable verse references
- **Hebrew calendar** — Messianic holidays displayed for each day
- **Start from any book** — jump to a specific book/chapter; the schedule shifts accordingly
- **Plan date shifting** — missed days? Shift the schedule with one tap
- **In-app Bible reader** — six translations (TLV, ESV, NASB, KJV, NKJV, NIV), red-letter support
- **Granular typography** — font size, line height, letter spacing, text alignment
- **Community groups** — create or join study groups with a shared plan timeline
- **Direct messages** — one-on-one encrypted messaging
- **End-to-end encryption** — RSA-OAEP 2048 + AES-256-GCM; private keys stay in your browser
- **Real-time chat** — WebSocket-powered group and DM chat
- **Cross-device sync** — progress and answers synced via Turso (libSQL)
- **Offline-capable** — service worker caches the app shell

## Repository structure

```
app/        Vite + React PWA → deploys to Vercel
server/     Express + ws API server → deploys to Fly.io
data/       Source data: plan.json (365 days), schedule.json
scripts/    Build scripts for regenerating data files
```
...
cd app
npm install
cp .env.example .env.local          # set VITE_API_URL=http://localhost:8787
npm run dev
...
3. Add env var: `VITE_API_URL=https://your-server.fly.dev`
4. Deploy
...
Each day contains four parallel tracks:

| Track | Content | Pace |
|---|---|---|
| `tanakh` | Torah → Nevi'im → Ketuvim in traditional Hebrew order | ~2 chapters/day |
| `psalm` | Psalms, cycling through the year | Cycles ~2.5× |
| `proverbs` | Proverbs 1–31 | Cycles ~11.8× |
| `brit_chadashah` | Matthew → Revelation, then Matthew–Acts again | Once through + partial repeat |

Each day also has a `theme` and five `questions` (observation, word_study, messianic_connection, application, discussion).

## Regenerating data

```bash
node scripts/build-schedule.js   # rebuilds data/schedule.json
```

`data/plan.json` (themes + questions) was authored per-day and is not regenerated by the script.

## Terminology

Yeshua, Messiah/Mashiach, Adonai, Torah, Tanakh, B'rit Chadashah, Ruach ha-Kodesh, and Hebrew names for B'rit Chadashah authors. Supersessionist framing is intentionally avoided — Israel's covenants are treated as standing.
